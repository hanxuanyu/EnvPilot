...
export function KafkaConnectorPanel({ asset }: { asset: Asset }) {
  ...
  const handleSend = async () => {
    ...
    try {
      ...
      setHistory(prev => [{ topic, key, result, createdAt: new Date().toLocaleString('zh-CN') }, ...prev].slice(0, 10))
      setStatus({ type: 'success', message: 'Kafka 消息发送成功' })
      toast.success('Kafka 消息发送成功')
    } catch (error: any) {
      setStatus({ type: 'error', message: error.message || 'Kafka 消息发送失败' })
      toast.error('Kafka 消息发送失败', { description: error.message })
    } finally {
      setSending(false)
    }
  }
        ...
        <div className="rounded-xl border border-border bg-card p-4 space-y-3">
          <div className="flex items-center justify-between gap-3">
            <div>
              ...
              <div className="text-xs text-muted-foreground">支持 topic、key 和 headers</div>
            </div>
          ...
          <div className="grid gap-3 md:grid-cols-2">
            ...
            <Input value={key} onChange={(e) => setKey(e.target.value)} placeholder="message key，可留空" />
          </div>
