...
- 资产：创建、更新、删除
- 凭据：创建、更新、删除、明文查看
- 健康检查：单资产检查、批量检查、自动调度摘要
- 连接器：连接测试、SQL 执行、Redis 命令执行、MQ 消息发送
- 执行器：命令执行结果、高危命令拦截、终端会话开启与关闭
- 配置：配置更新、配置回滚
...
- 命令执行当前仅记录命令摘要、退出码和输出长度，不直接持久化完整输出内容
- 后续扩展到 config 等剩余链路时，优先沿用同一个 `AuditService`
...
- 健康快照持久化与最新状态查询
- 启动后自动周期检查与手动单资产/批量检查
- 服务器类资产 SSH 只读指标采集
- 数据库、缓存、消息队列类资产的 Connector Ping 与元数据只读探测
- 汇总统计、状态聚合与前端看板展示

当前能力：

- 快照状态覆盖 `healthy`、`warning`、`critical`、`unreachable`、`unknown`
- 服务器指标已覆盖主机名、内核、CPU 核心、负载、内存、磁盘、运行时长
- 中间件类资产优先复用 `connector` 模块能力，避免重复实现协议探测
- 已提供 Wails API 与 HTTP API，并接入前端健康页面和资产列表概览

目录职责：

- `model/`：健康快照模型与指标结构
- `repository/`：快照写入、最新快照分页查询与统计
- `service/`：检查编排、调度器、状态聚合与指标采集
- `api/`：Wails 绑定接口
