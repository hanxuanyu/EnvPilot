...
**当前状态**：进行中，已完成 DNS 记录数据模型、双模式 API、管理页面基础能力、miekg/dns 运行时、查询日志链路，以及资产创建/编辑时的自动 DNS 联动维护
...
- [x] Task 5.1 — DNSRecord 模型与数据库迁移（`005_dns`）
- [x] Task 5.2 — DNS 记录 Repository/Service/API
  - 已支持资产创建时自动创建关联 A 记录
  - 已支持按环境名、资产类型、资产名称推荐默认域名
- [x] Task 5.3 — DNS 服务器实现（`miekg/dns`）
  - A/CNAME 记录解析
  - 按环境隔离逻辑
  - 未命中时转发到上游 DNS
- [x] Task 5.4 — DNS 查询日志记录
- [x] Task 5.5 — DNS 管理页面 UI（域名列表 + 添加/编辑 + 运行状态 + 查询日志）

### 本轮补充

- 已在容器启动阶段接入 DNS runtime，并在清理阶段停止服务
- 已新增 DNS 查询日志表与查询接口，用于查看来源、响应码、耗时与命中情况
- 已支持资产编辑时维护关联 DNS：可创建、更新或删除资产绑定的 A 记录
- 已在 DNS 页面展示运行状态、监听地址、上游配置与最近查询日志
...
- [x] 可为指定环境创建和管理 A/CNAME 记录
- [x] 查询请求可按环境隔离解析，不串环境
- [x] 未命中记录时可安全转发到上游 DNS
- [x] DNS 查询日志可回溯具体域名与结果
- [x] 前端页面可完成记录维护与查询日志查看
